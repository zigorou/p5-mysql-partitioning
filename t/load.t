...
sub test_partitions {
    ...
    is_deeply( $got, $expects, 'partitions fields' );
}

sub test_load {
    my %specs = @_;
    my ( $input, $expects, $desc ) = @specs{qw/input expects desc/};
    my ( $dbh, $table ) = @$input{qw/dbh table/};
    my ($expected_partitions) = @$expects{qw/partitions/};
    ...
    subtest $desc => sub {
        ...
        lives_ok { $p->load } 'load() lives ok';
        test_partitions( $p->partitions, $expected_partitions );
...
test_load(
    desc  => 'employees2 (LIST partition)',
    input => +{
        dbh   => $dbh_args,
        table => 'employees2',
    },
...
test_load(
    desc  => 'employees3 (RANGE partition)',
    input => +{
        dbh   => $dbh_args,
        table => 'employees3',
    },
